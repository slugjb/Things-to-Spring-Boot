...
* GET
  ...
  * 길이 제한 O
    *  get 요청에 의해 url이 너무 길다고 서버단에서 판단하면 414 오류를 보내는 프로토콜이  
       지정되어 있을뿐, 길이에 대해 명확한 부분은 없다.  
       서버 기준이 아닌, 브라우저의 기준에 따른다.
  * 데이터 요청에만 사용
  * 보안 매우 취약
  * http 메시지에 body 없음
  * 멱등(idempotent)이다
...
## Autowired `보충필요`
  ...
  Autowired는 기본값이 ture이기 때문에 의존성 주입을 해야할 대상을 찾지 못한다면 애플리케이션 구동에 실패한다.
...
```
 ...
 위에 정의한 POM은 메이븐이 허용한 최소값이다. 모두 필수 필드이지만,  
 groupId, artifactId, verson은 상위 항목에서 상속된 경우 명시적으로 정의할 필요가 없다. 
...
```
* properties(선택사항)
> pom.xml 내에서 자주 사용되는 정보를 변수처럼 만들어서 사용할 수 있다.  
> 같은 내용을 여러번 쓰는 경우에도 이렇게 사용할 수 있지만, 프로젝트 설정의  
...
### dependencies 하위 필드
   ...
   * scope
      * 해당 라이브러리가 적용될 범위를 지정할 수 있다. 사용 가능한 범위는 5개가 있다.
         ...
         * compile(default)
            ...
            * 모든 클래스 경로에서 사용할 수 있고, 컴파일 및 배포시 같이 제공된다.
   ...
   * profiles(선택사항)
      ...
      > 주로 로컬, 개발서버, 운영서버를 나누어 배포하여야 하는 설정 등이 존재하는데,  
...
```
@SpringBootApplication이 존재하며 안을 확인해보면 아래와 같다.
```
...
public @interface SpringBootApplication {
    ...
```    
위와 같이 구성되어 있다. @SpringBootApplication은
...
## RequiredArgsConstructor
  ...
  주로 의존성 주입(Dependeny Injection)편의성을 위해서 사용된다.   
  스프링 의존성 주입의 특징 중 한가지를 이용한다.
  
 > 어떠한 Bean에 생성자가 오직 하나만 있고, 생성자의 파라미터 타입이  
 > 빈으로 등록 가능한 존재라면, 이 빈은 @Autowired 어노테이션 없이도 의존성 주입이 가능하다.
...
## Log4j / Slf4j
  ...
  * log4j
    ...
    [참고링크1](https://cheershennah.tistory.com/80), [참고링크2](https://cofs.tistory.com/354)
